...
// import ".ServicePage.css";
import { FC, useEffect, useState } from "react";
import {ServicePageProps} from "./typing.tsx";
import { useParams } from "react-router-dom"; 
import { Container, Card, Col, Row, Image } from "react-bootstrap";
import { ServiceList_ } from "../../core/mock/RentList";
import { Navbar } from "../../components/Navbar/index.tsx";
import { Breadcrumbs } from "../../components/BreadCrumbs/typing.tsx";
...
export const ServicePage: FC<ServicePageProps> = () => {
    ...
    useEffect(() => {
        if (id) {
            api.rentServices.rentServicesRead(id)
                .then((data) => {
                    setServiceData(data.data);
                    setEndItem(data.data.title); // Устанавливаем `endItem` после загрузки
                })
                .catch(() => {
                    const service = ServiceList_.find(
                        (service) => service.pk === Number(id)
